...
**状態: 人間QA未実施（balance revision 5 source gate PASS、正式候補未固定）**
...
実測CSVとsummaryは`artifacts/balance/revision-5/`に保存している。このPASSは自動調整の完了記録であり、
...
- 全GDScript回帰: PASS（2026-09-02、報酬種別表示追記後112/112）
- GDScript guard: PASS（97ファイル）
- 変更GDScript check-only: PASS（revision 5実装時41/41、報酬種別表示変更2/2）
- 候補固定前のrevision 5専用12run source gate: PASS（上記実測値）
...
- 敵の経験値結晶を取得すると戦闘が停止し、重複しない最大3候補を選べる。各候補は名前、レベル、
  `種別：武器`または`種別：パッシブ`、説明、進化情報の順に表示される。リロール、スキップ、除外はない。
