...
**状態: 人間QA未実施（balance revision 6 source gate未実施、正式候補未固定）**
...
## revision 6 source gate

未実施。基本武器のLv2〜Lv8を各レベル1強化項目へ変更したため、revision 5の自動調整結果は流用しない。
敵、XP、segment、boss、候補抽選、進化条件は変更していないが、武器性能に合わせた再調整を別作業で行う。

## revision 5自動調整済みsource gate（履歴・revision 6へ流用禁止）
...
空欄のrevision 6正式candidate identityに対する性能試験、Release検証、ManualQa、
人間playtestの完了を意味しない。
...
- 全GDScript回帰: PASS（2026-09-02、revision 6単一強化項目実装後114/114）
- GDScript guard: PASS（98ファイル）
- 変更GDScript check-only: PASS（revision 6の変更15/15）
- revision 6専用12run source gate: 未実施
- revision 5専用12run source gate: PASS（履歴専用、revision 6へ流用禁止）
...
- 10:00に通常スポーンが止まり、通常敵と既存enemy projectileが即座に無害化・吸収される。この吸収ではXP、撃破数、CHAIN、dropを得ず、落下済みXPとarena objectは残る。
