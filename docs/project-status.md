...
戦闘中のWASD、矢印キー、方向パッド、左スティックは、斜め俯瞰カメラの向きから画面基準の
上下左右へ変換する。通常戦闘とW1チュートリアルが同じ変換を使い、入力強度、世界内の移動速度、
アリーナ境界での壁沿い移動は維持する。
...
現行GDScript全回帰は `TEST_SUMMARY passed=60 failed=0`。装備、戦利品、合成、戦闘、UI、score、
