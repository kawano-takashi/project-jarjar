...
**状態: 使用禁止（balance revision 9 source gate未実施・正式候補未固定）**
...
revision 9 source gateは未実施である。通常敵と50体群れの生成枠をプレイヤー追従化し、通常敵へ遠方破棄を追加したため、
下記の旧結果は履歴としてのみ保持し、revision 9の正式対象固定には使わない。
...
同じrevision 5作業ツリーで全GDScript回帰112/112もPASSしたが、revision 9の証拠としては無効である。
