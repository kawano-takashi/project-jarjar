...
`JarjarCombatWorld` と幾何計算用 `JarjarCombatKernel` はRefCountedで、APIバージョン2を起動時に検査します。位置・半径の単位はメートル、時間は整数tick（60tick/秒）です。敵IDは64bit整数、弾のハンドルは `[slot, generation]` の `PackedInt64Array` です。解放・再利用・容量変更後も以前のハンドルで新しい弾を操作できません。プールの設定値はmanifest内の `CombatBalanceDefinition` から読み込み、描画容量は実際の個体数に合わせて拡張します。
