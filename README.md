...
接触と敵弾を同じ候補群として毎combat tick（固定60Hz）判定し、倍率適用後の最大威力1件だけを適用します。
通常被弾後の保護は廃止したため、接触中は初回から毎tick再被弾し、離脱後の再接触でも即時再開します。
衝突した敵弾は、不採用またはレベルアップ復帰保護中でもすべて消費します。レベルアップ画面から直接戦闘へ
戻る場合だけ次の45 combat tickを保護し、宝箱からの復帰やレベルアップ後に宝箱で終わる列には付与しません。
実ダメージごとに被弾イベントを発行し、音・振動の実出力は既存の最大8回/秒を維持します。

revision 13で導入した円形の継続接触とソフト接触も維持します。通常追尾敵、エリート、ボスはプレイヤーとの
合成半径で止まり、プレイヤーは速度を落とさずに食い込んだ敵を必要最小距離だけ押し分けます。
完全同位置ではentity ID由来の決定方向、壁際では敵境界へのclampを使います。敵別の接触周期フィールドと
通常被弾用の保護フィールドは削除済みです。

revision 12でrevision 11からプレイヤーと通常敵の座標移動速度を厳密にさらに10%下げた値を維持し、Player 4.05、Pursuer 1.944、
...
50体群れはソフト接触の対象外でプレイヤーを通過し、revision 11の`2.59m/s`を維持します。Hyper Mad Forestの公開映像2本から
...
revision 14実装後の全GDScript回帰141/141、全8基本武器のLv1接触前撃破fixture 8/8、GDScript guard 103ファイル、
変更したGDScriptのcheck-only 14/14、全リソース172件の事前ロード、差分の空白検査がPASSしています。fixtureは1:00〜2:00の通常Swarmer
...
revision 14の正式12run source gateは未実施で、自動難易度は未評価です。revision 13以前の検証結果は履歴扱いです。
正式candidateと正式playtest targetは未固定で、
人間の体感確認待ちです。Full HD性能試験、Release export、Verify、ManualQa、人間playtestも実施していません。

## revision 5自動調整の履歴（revision 14へ流用禁止）
...
これらはrevision 5の履歴値であり、revision 14の実データや現候補の証拠ではありません。
