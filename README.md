...
プレイヤーと敵の円形重なりを毎tick判定する継続接触ダメージを導入しました。通常追尾敵、エリート、ボスは
プレイヤーとの合成半径で止まり、プレイヤーは速度を落とさずに食い込んだ敵を必要最小距離だけ押し分けます。
完全同位置ではentity ID由来の決定方向、壁際では敵境界へのclampを使います。接触中は初回即時、以後は
全攻撃源共通の30 combat tick（0.5秒）保護が終わるたびに再被弾します。同tickの全接触と敵弾は、倍率適用後の
最大威力1件だけを採用し、衝突した敵弾は無敵中や不採用でも消費します。敵別の接触周期フィールドは削除しました。
...
revision 13実装後の全GDScript回帰138/138、全8基本武器のLv1接触前撃破fixture 8/8、GDScript guard 103ファイル、
...
これらはrevision 5の履歴値であり、revision 13の実データや現候補の証拠ではありません。
