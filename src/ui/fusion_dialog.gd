...
const ACTION_COUNT: int = 3
const MATERIAL_CANDIDATE_COLUMNS: Array[int] = [0, 3, 6]
const ACTION_CANDIDATE_COLUMNS: Array[int] = [1, 4, 6]
...
func _ready() -> void:
	_rarity.pressed.connect(func() -> void: rarity_step_requested.emit(1))
	for index: int in range(_material_slots.size()):
		...
		slot.pointer_event.connect(func() -> void: pointer_event.emit())
	...
	_remove_zone.pointer_event.connect(func() -> void: pointer_event.emit())
	visible = false
...
func update_view(
	...
	for index: int in range(MATERIAL_COUNT):
		var entry: Dictionary = (
			material_entries[index].duplicate(true)
			if index < material_entries.size()
			else {"slot_index": index, "item_id": "", "item": null, "details": "材料枠%d、空き" % (index + 1)}
		)
		...
		slot.configure_item_visual(
			...
			str(entry.get("details", "材料枠%d、空き" % (index + 1))),
		)
		slot.configure_drag(
			{"drag_type": &"fusion_material", "slot_index": index, "item_id": item_id},
			{},
...
func focus_after_success() -> bool:
	return _focus_controller.grab_focus_id("FA0")
...
func test_accept() -> void:
	var control: Control = _focus_controller.control_for_id(
...
func _input(event: InputEvent) -> void:
	...
	if get_viewport().gui_get_focus_owner() == _rarity and not event.is_echo():
...
func _sync_candidates(candidate_entries: Array[Dictionary]) -> void:
	...
	for index: int in range(_candidate_cards.size()):
		...
		card.configure_drag(
			{"drag_type": &"item", "kind": source.get("kind", &""), "index": source.get("index", -1), "item_id": entry.get("item_id", "")},
			{},
			true,
			&"",
		)
...
func _rebuild_candidate_cards(candidate_entries: Array[Dictionary]) -> void:
	...
	for index: int in range(candidate_entries.size()):
		...
		card.pointer_event.connect(func() -> void: pointer_event.emit())
...
func _configure_focus_graph() -> void:
	...
	graph["FR"] = _focus_graph_entry("FA2", "F0", "FR", "FR")
	for slot_index: int in range(MATERIAL_COUNT):
	...
	for index: int in range(_candidate_cards.size()):
		...
		var top: String = (
			_material_id_for_candidate_column(column)
			if row == 0
			else "FC%d" % (index - CANDIDATE_COLUMNS)
		)
		var bottom: String = (
	...
	for index: int in range(ACTION_COUNT):
		...
		graph[focus_id] = _focus_graph_entry(
			top,
			"FR",
			"FA%d" % posmod(index - 1, ACTION_COUNT),
			"FA%d" % ((index + 1) % ACTION_COUNT),
		)
...
func _candidate_id_for_material(material_index: int) -> String:
	if _candidate_cards.is_empty():
		return ""
	return "FC%d" % mini(
		MATERIAL_CANDIDATE_COLUMNS[material_index],
		_candidate_cards.size() - 1,
	)
...
func _last_candidate_id_for_action(action_index: int) -> String:
	...
	var candidate_index: int = (
		last_row * CANDIDATE_COLUMNS + ACTION_CANDIDATE_COLUMNS[action_index]
	)
	return "FC%d" % mini(candidate_index, _candidate_cards.size() - 1)
...
func _action_id_for_material(material_index: int) -> String:
	return "FA%d" % material_index


func _material_id_for_action(action_index: int) -> String:
	return "F%d" % action_index


func _action_control(index: int) -> Control:
	return [_auto_fill, _confirm, _cancel][index] as Control
...
func _material_tooltip_payload(index: int) -> Dictionary:
	if index < 0 or index >= _material_entries.size():
		return {}
	return {"details": str(_material_entries[index].get("details", "")), "warning": ""}
...
func _candidate_tooltip_payload(item_id: String) -> Dictionary:
	...
	if index < 0 or index >= _candidate_entries.size():
		return {}
	return {"details": str(_candidate_entries[index].get("details", "")), "warning": ""}


func _on_material_item_dropped(source: Dictionary, _target: Dictionary, slot_index: int) -> void:
	material_item_dropped.emit(source, slot_index)


func _on_material_drop_removed(source: Dictionary) -> void:
	if StringName(source.get("drag_type", &"")) == &"fusion_material":
