...
func _validate_globals() -> void:
	if _manifest.balance == null or _manifest.balance.balance_revision != 11:
		_add_error("balance revision must be 11")
...
func _validate_enemies() -> void:
	...
	if elite == null or not elite.drops_chest:
		...
		_add_error("revision 11 elite HP must remain 650")
	if boss == null or not boss.is_boss:
		_add_error("boss definition must be marked as boss")
	_validate_revision_eleven_enemy_roles()


func _validate_revision_eleven_enemy_roles() -> void:
	var expected_specs: Dictionary[StringName, Array] = {
		&"pursuer": [20.0, 2.16, 45, 8.0, 1],
		&"swarmer": [9.0, 5.76, 45, 4.0, 1],
		&"bulwark": [80.0, 1.215, 60, 14.0, 2],
		&"shooter": [34.0, 2.7, 42, 11.0, 2],
		&"elite": [650.0, 1.8, 45, 18.0, 50],
		&"boss": [40000.0, 1.44, 40, 24.0, 0],
	}
	for enemy_id: StringName in expected_specs:
		...
		if (
			...
			_add_error("revision 11 enemy role differs from approved values: %s" % enemy_id)


func _validate_swarm_event() -> void:
	var event_definition: SwarmEventDefinition = _manifest.swarm_event
	if event_definition == null:
		_add_error("revision 11 bat swarm definition is required")
		return
	...
	if unit == null:
		...
		if (
			...
			_add_error("revision 11 bat swarm unit differs from approved values")
		if (
	...
	if (
		...
		or not is_equal_approx(event_definition.depth_pitch, 0.7)
			...
			_add_error("revision 11 bat swarm formation differs from approved values")
...
func _validate_segments() -> void:
	...
	for index: int in range(10):
		...
		if definition.target_active != EXPECTED_TARGETS[index]:
			_add_error("segment target differs from revision 11 value: %d" % index)
		if not is_equal_approx(definition.hp_multiplier, BASELINE_HP_MULTIPLIERS[index]):
			_add_error("segment HP differs from revision 11 value: %d" % index)
		if not is_equal_approx(
			definition.damage_multiplier,
			BASELINE_DAMAGE_MULTIPLIERS[index],
		):
			_add_error("segment damage differs from revision 11 value: %d" % index)
		...
		if not _float_arrays_equal(definition.spawn_weights, EXPECTED_SEGMENT_WEIGHTS[index]):
			_add_error("segment weights differ from revision 11 value: %d" % index)
