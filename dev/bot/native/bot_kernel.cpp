...
double escape_score(V2 direction, const Frame &f, double baseline) {
    if (!f.escape_active) return baseline;
    double closest = INF;
    for (const Body &enemy : f.enemies)
        closest = minimum(closest, enemy.relative.length() - enemy.radius - f.radius);
    if (closest >= 0.6) return baseline;
    if (direction.length_squared() < 0.5) return -1.0e13;
    V2 velocity = direction * f.move_speed;
    double danger = 0.0;
    for (const Body &enemy : f.enemies) {
        if (enemy.relative.length_squared() > 36.0) continue;
        ...
        for (int step = 1; step <= 60; ++step) {
            ...
            if (enemy.fixed) {
                ...
                if (distance < radius) position = player + away.normalized() * radius;
                else position -= away.normalized() * minimum(enemy.speed / 60.0, distance - radius);
            }
            double clearance = position.distance_to(player) - radius;
            if (clearance < 0.05) danger += 1.0 + double(60 - step) / 60.0;
...
double movement_score(V2 direction, const Frame &f) {
    ...
    V2 warning_destination = player + velocity * minimum(travel_time, 0.25);
    ...
    return escape_score(direction, f, score);
}
}
...
public:
    int api_version() const { return 4; }
    ...
    PackedFloat64Array score_moves(const Dictionary &data, const PackedVector2Array &directions) {
        ...
        for (const Track &track : enemies) {
            ...
            f.enemies.push_back({track.position, relative, avoidance, track.velocity, track.radius, inferred_speed, track.kind, track.fixed, track.materializing});
        }
    ...
    Vector2 aim_needles(const PackedVector2Array &directions, int count, Vector2 navigation_move, double reach) const {
        ...
        return aim.zero() ? navigation_move : (aim * 0.02).value();
    }
};
