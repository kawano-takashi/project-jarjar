...
func test_chest_kind_survives_delayed_elite_death_and_collection(a: Variant, _context: Dictionary) -> void:
	...
	var first: EnemyEntity = system.resolve_stage_events(Vector2.ZERO, 1)[0]
	...
	a.expect_equal(2, system.resolve_stage_events(Vector2.ZERO, 6).size(), "later elites retain separate scheduled sources")
...
func test_swarm_warning_locks_route_and_consumes_busy_attempts(a: Variant, _context: Dictionary) -> void:
	var sim: CombatSimulation = _simulation(_catalog(_content(), a))
	var system: EnemySystem = sim.enemy_system
	system.resolve_stage_events(Vector2.ZERO, 5)
	var anchor := Vector2(2, -3)
	sim.state.combat_tick = 10
	a.expect_true(system.resolve_stage_events(anchor, 10).is_empty(), "attempt starts warning without spawning enemies")
	var warning: SwarmWarningState = system.stage_events.swarm_warning
	...
	for tick: int in [12, 14]:
		...
		a.expect_true(system.resolve_stage_events(-anchor, tick).is_empty(), "busy warning consumes extra attempts without a second route")
		a.expect_true(system.stage_events.swarm_warning == warning, "warning is not replaced when the player moves")
	sim.state.combat_tick = 15
	var group: Array[EnemyEntity] = system.resolve_stage_events(-anchor, 15)
	a.expect_equal(3, group.size(), "exact deadline spawns the complete group")
	a.expect_true(system.stage_events.swarm_warning == null, "warning ends on spawn")
	...
	system.resolve_stage_events(Vector2.ZERO, 16)
	a.expect_true(system.stage_events.swarm_warning == null, "living swarm prevents another warning")
	...
	system.resolve_stage_events(Vector2.ZERO, 24)
	a.expect_true(system.stage_events.swarm_warning == null, "clearing a group does not release a backlog")
	a.expect_equal(1, sim.state.swarm_event_group_count, "skipped groups never spawn later")
	system.resolve_stage_events(Vector2.ZERO, 25)
	a.expect_true(system.stage_events.swarm_warning != null, "next fresh schedule can start after the group is gone")
...
func test_swarm_warning_pauses_and_is_cancelled_at_short_boss_boundary(a: Variant, _context: Dictionary) -> void:
	var sim: CombatSimulation = _simulation(_catalog(_content(), a))
	sim.state.combat_tick = 10
	sim.enemy_system.resolve_stage_events(Vector2.ZERO, 10)
	var warning: SwarmWarningState = sim.enemy_system.stage_events.swarm_warning
	...
	sim.enemy_system.stage_events.swarm_warning = null
	sim.enemy_system.enemy_store.clear()
	sim.state.combat_tick = 78
	sim.enemy_system.resolve_stage_events(Vector2.ZERO, 78)
	a.expect_true(sim.enemy_system.stage_events.swarm_warning != null, "late warning starts before shortened boss boundary")
	...
	a.expect_true(sim.enemy_system.stage_events.swarm_warning == null, "boss transition cancels warning even before its deadline")
	a.expect_equal(0, sim.enemy_system.resolve_stage_events(Vector2.ZERO, 80).size(), "boss is not spawned twice")
	sim.advance_tick(Vector2.ZERO)
...
func test_swarm_kills_open_a_gap(a: Variant, _context: Dictionary) -> void:
	...
	var group: Array[EnemyEntity] = sim.enemy_system.spawn_swarm_group(Vector2.ZERO, Vector2.RIGHT, 20, 2.0, 3.0, 4.0)
...
func test_chest_shapes_and_swarm_warning_match_gameplay_snapshot(a: Variant, context: Dictionary) -> void:
	...
	sim.state.combat_tick = 10
	sim.enemy_system.resolve_stage_events(Vector2(3, 4), 10)
...
func test_elite_and_swarm_resources_reject_invalid_schedules(a: Variant, _context: Dictionary) -> void:
	for invalid_case: int in range(9):
		var content: SurvivalContentManifest = _content()
		var field: String
		match invalid_case:
			0:
				content.stage_events.events[0] = null
				field = "events"
			1:
				content.stage_events.events[0].start_tick = 80
				field = "start_tick"
			2:
				content.stage_events.events[0].set("chest_kind", 99)
				field = "chest_kind"
			3:
				content.swarm_event.telegraph_ticks = 0
				field = "telegraph_ticks"
			4:
				(content.stage_events.events[3] as SwarmEventScheduleDefinition).hp_multiplier = 0.0
				field = "hp_multiplier"
			5:
				content.stage_events.events[3].event_id = content.stage_events.events[0].event_id
				field = "event_id"
			6:
				(content.stage_events.events[3] as SwarmEventScheduleDefinition).interval_ticks = 40
				field = "attempt_count"
			7:
				var unsupported := StageEventDefinition.new()
				unsupported.event_id = &"unsupported"
				content.stage_events.events.append(unsupported)
				field = "event"
			8:
				content.stage_events = null
				field = "stage_events"
...
func _content() -> SurvivalContentManifest:
	...
	content.stage_events.events = BalanceTestFixtures.elite_events([1, 3, 5])
	(content.stage_events.events[1] as EliteSpawnDefinition).chest_kind = GameTypes.ChestKind.EVOLUTION_CAPABLE
	...
	content.stage_events.events.append(_schedule(&"dense", 10, 2, 4))
	content.stage_events.events.append(_schedule(&"fresh", 25, 1, 1))
	content.stage_events.events.append(_schedule(&"late", 78, 1, 1))
	return content
