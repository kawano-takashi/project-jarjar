...
func test_names() -> PackedStringArray:
	return PackedStringArray([
		...
		"launch_balance_and_engine_contracts_are_revision_eleven",
	])
...
func _test_launch_balance(assertions: Variant) -> void:
	...
	if catalog.is_valid:
		assertions.expect_equal(11, catalog.balance_manifest().balance_revision, "balance revision is eleven")
