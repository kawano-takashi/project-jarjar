...
func test_names() -> PackedStringArray:
	return PackedStringArray([
		...
		"combat_event_router_serial_contract",
	])
...
func run_test(test_name: String, assertions: Variant, _context: Dictionary) -> void:
	match test_name:
		...
		"combat_event_router_serial_contract":
			_test_combat_event_router_serial_contract(assertions)
		_:
...
func _test_uniform_grid_segment_and_weapon_queries(assertions: Variant) -> void:
	...
	staff_grid.insert(30, Vector2(4.58, 0.0))
	assertions.expect_true(
		30 in staff_grid.query_circle_candidates(Vector2.ZERO, 3.33, 1.25),
		"staff padded AABB reaches boss in another cell",
	)
	assertions.expect_true(
		Vector2(4.58, 0.0).x > 3.33,
		"boss center lies outside effect-only AABB",
	)
	assertions.expect_true(
		CombatGeometry.circle_intersects(Vector2.ZERO, 3.33, Vector2(4.58, 0.0), 1.25),
		"staff maximum area plus boss radius boundary hits",
	)
	assertions.expect_false(
		CombatGeometry.circle_intersects(Vector2.ZERO, 3.33, Vector2(4.5801, 0.0), 1.25),
...
func _test_combat_event_router_serial_contract(assertions: Variant) -> void:
	var state := RunState.new()
	var router := CombatEventRouter.new()
	var first: CombatEvent = router.create_primary(
		state,
		&"damage",
		1,
		&"weapon:bow",
	...
	var second: CombatEvent = router.create_primary(state, &"damage", 2, &"weapon:staff", 18.0)
	assertions.expect_equal(0, first.event_serial, "first event serial starts at zero")
	assertions.expect_equal(1, second.event_serial, "second event serial increments")
	assertions.expect_equal(2, state.next_event_serial, "run state owns deterministic event serial")
	assertions.expect_equal(&"weapon:bow", first.source_effect_id, "event keeps per-item source")
	assertions.expect_float(12.0, first.damage_snapshot, "event snapshots damage")
	assertions.expect_equal(Vector2(2.0, 3.0), first.position, "event snapshots position")
	assertions.expect_equal(Vector2.RIGHT, first.direction, "event snapshots direction")
